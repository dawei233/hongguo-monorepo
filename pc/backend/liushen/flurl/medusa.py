...
def gen_medusa_proto(url, url_params, devices, data:dict, khronos:int=None, lanusk=None, dataType=None):
    ...
    query_sm3 = SM3(query_part.encode() if isinstance(query_part, str) else query_part).digest()
    if len(lanusk) > 0:
        ...
        psk_version = "1"
        query_body_hash_sm3 = SM3(query_part.encode() + body_md5_bytes + b"1").digest()
    else:
        lanusk_hash = b''
        psk_version = 'none'
        query_body_hash_sm3 = SM3(query_part.encode() + body_md5_bytes + b"none").digest()
