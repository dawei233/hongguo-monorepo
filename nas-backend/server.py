...
# server.py 用作 CDN/player 页 scraping 等网络调用。
# _requests 保留仅用于少数特殊场景（如生成 multipart 流式下载），
# 普通 GET 一律走 hongguo_core.SESSION。
...
def api_prefs():
    """用户数据（收藏/播放记录）持久化：存后端 prefs.json，不依赖浏览器 profile"""
    try:
        with _prefs_lock:
...
# v52: AI 智能搜剧 —— 自然语言描述 → 智谱 GLM 提取关键词 → 走现有 search_series
# 用法: GET /api/search_ai?q=想看重生复仇类短剧&tab=1
...
def api_search_ai():
    ...
    # 在结果中回传 AI 提取的关键词,前端可显示"AI 解读: 关键词1 关键词2"
    return jsonify({
        "ok": True,
        "ai_query": q,
        "ai_keywords": kws,
...
def api_play():
    ...
    if mode != "download":
        # v52: 三段找源逻辑抽到 hongguo_core.resolve_with_fallback,
        # 60 行 → 3 行,两份 server.py 不再重复(同步到 pc)
        try:
...
# v52: /api/cdn 代理域名白名单 —— 之前对 u= 参数无任何校验，局域网设备可把 NAS 当任意 HTTP 代理跳板。
# 允许后缀匹配：.hongguoduanju.com / .fqnovel.com / .byteicdn.com / .byteicdn.cn / .douyinpic.com 等。
# CDN host 关键词片段（reading-videocdn / hgweb / v11- / v26- / v3-hgweb）通过 contains 命中
# —— 这些是 hongguo_core.resolve_video_url 解析出的实际播放 CDN host 关键词。
# 实际跑一段时间后看日志补全白名单；非法域名直接 403。
CDN_ALLOW_SUFFIXES = (
    "hongguoduanju.com",
    "fqnovel.com",
    "douyinpic.com",
...
CDN_ALLOW_KEYWORDS = (
    "reading-videocdn", "hgweb", "douyinvod", "byteicdn",
    "v11-", "v26-", "v3-hgweb",
)


def _is_cdn_host_allowed(host: str) -> bool:
    """v52: 校验 host 是否在 CDN 白名单内。host 必须非空。"""
    if not host:
        return False
    h = host.lower()
    if any(h.endswith("." + suf) or h == suf for suf in CDN_ALLOW_SUFFIXES):
        return True
    if any(k in h for k in CDN_ALLOW_KEYWORDS):
        return True
    return False
...
def _serve_production(app, host: str, port: int) -> None:
    """v52: 生产级 WSGI 替换 Werkzeug dev server。

    优先 waitress（纯 Python、Windows 友好、PyInstaller 打包无障碍；并发处理/超时/
    断连行为都更稳，已知能解决 dev server 在 206+chunked 上的边界 bug）。
    waitress 不可用时回退到 Flask 自带 dev server。
    """
    try:
        from waitress import serve
        print(f"[serve] 使用 waitress 生产服务器")
        serve(app, host=host, port=port, threads=8, ident="hongguo-backend")
        return
    except ImportError:
        pass
